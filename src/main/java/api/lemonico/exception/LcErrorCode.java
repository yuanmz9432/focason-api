...
public enum LcErrorCode {
    ...
    LcErrorCode(final String value) {
        this.value = value;
    }

    @JsonCreator
    public static LcErrorCode of(String value) {
        return Arrays.stream(values()).filter((v) -> v.value.equals(value)).findFirst().orElseThrow(() -> new IllegalArgumentException("RzErrorCode = '" + value + "' is not supported."));
    }
