...
import api.lemonico.dao.FaceDao;
...
public class FaceRepository
{
     ...
     */
    public LcResultSet<Face> findAll(Condition condition, LcPagination pagination, Sort sort) {
     ...
     */
    public Optional<Face> findById(ID<Face> id) throws IllegalArgumentException {
     ...
     */
    public ID<Face> create(Face entity) {
     ...
     * @param entity エンティティ
     */
    public void update(ID<Face> id, Face entity) {
        Objects.requireNonNull(entity, "'entity' must not be NULL.");
        var result = dao.update(entity.withId(id));
        if (result.getCount() != 1) {
            throw new LcEntityNotFoundException(Face.class, entity.getId());
        }
    }
     ...
     * @param id エンティティID
     */
    public void deleteById(ID<Face> id) throws IllegalArgumentException {
        var deleted = dao.deleteById(id);
        if (deleted != 1) {
            throw new LcEntityNotFoundException(Face.class, id);
        }
    }
     ...
     * @param id エンティティID
     */
    public void deleteLogicById(ID<Face> id) throws IllegalArgumentException {
     ...
     */
    public boolean exists(ID<Face> id) {
