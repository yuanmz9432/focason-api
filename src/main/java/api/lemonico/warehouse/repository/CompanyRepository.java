...
import api.lemonico.warehouse.dao.CompanyDao;
...
public class CompanyRepository
{
     ...
     */
    public LcResultSet<CompanyEntity> findAll(Condition condition, LcPagination pagination, Sort sort) {
     ...
     */
    public Optional<CompanyEntity> findById(ID<CompanyEntity> id) throws IllegalArgumentException {
     ...
     */
    public ID<CompanyEntity> create(CompanyEntity entity) {
     ...
     * @param entity エンティティ
     */
    public void update(ID<CompanyEntity> id, CompanyEntity entity) {
        Objects.requireNonNull(entity, "'entity' must not be NULL.");
        var result = dao.update(entity.withId(id));
        if (result.getCount() != 1) {
            throw new LcEntityNotFoundException(CompanyEntity.class, entity.getId());
        }
    }
     ...
     * @param id エンティティID
     */
    public void deleteById(ID<CompanyEntity> id) throws IllegalArgumentException {
        var deleted = dao.deleteById(id);
        if (deleted != 1) {
            throw new LcEntityNotFoundException(CompanyEntity.class, id);
        }
    }
     ...
     * @param id エンティティID
     */
    public void deleteLogicById(ID<CompanyEntity> id) throws IllegalArgumentException {
     ...
     */
    public boolean exists(ID<CompanyEntity> id) {
    ...
    public static class Condition
    {
         ...
         * 会社IDのセット（完全一致、複数指定可）
         */
        private Set<ID<CompanyEntity>> ids;
